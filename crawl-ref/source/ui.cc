...
namespace ui
{
    ...
    {
        return get_action(keyin) == CMD_MENU_EXIT || extended && keyin == CK_ENTER; // ugh
    }
    ...
    void Box::_allocate_region()
    {
        ...
        for (size_t i = 0; i < m_children.size(); i++)
        {
            ...
            case Widget::START:
                ...
                break;
            case Widget::CENTER:
                xo = extra_cross_space / 2;
                break;
            case Widget::END:
                xo = extra_cross_space;
                break;
            case Widget::STRETCH:
                xo = 0;
                break;
    ...
    void Text::wrap_text_to_size(int width, int height)
    {
        ...
        {
            const bool cached_width_max = m_wrapped_sizereq.width <= 0 || m_wrapped_sizereq.width > m_wrapped_size.width;
            const bool cached_height_max = m_wrapped_sizereq.height <= 0 || m_wrapped_sizereq.height > m_wrapped_size.height;
            if ((width == m_wrapped_size.width || cached_width_max && (width <= 0 || width >= m_wrapped_size.width)) && (height == m_wrapped_size.height || cached_height_max && (height <= 0 || height >= m_wrapped_size.height)))
            {
                return;
            }
        }
    ...
    void Text::_render()
    {
        ...
        if (!hl_pat.empty())
        {
            ...
            for (unsigned i = 0; i < slice.ops.size() && !highlights.empty(); i++)
            {
                ...
                const size_t op_y =
                    m_font->string_height(line_before_op.c_str(), false) - dev_line_height;
                ...
                if (start < oplen) // assume start is unsigned and so >=0
                {
                    ...
                    sy = m_font->string_height(before.c_str(), false) - dev_line_height;
                    started = true;
                }
                         ...
                         y += dev_line_height)
                    {
                        ...
                        if (hl_line)
                        {
                            ...
                            m_hl_buf.add(region.x,
                                         ...
                                         display_density.device_to_logical(y + dev_line_height),
                ...
                {
                    lacc += oplen;
                    line += m_font->string_height(op.text.c_str(), false) - dev_line_height;
                }
            }
        }
        ...
        m_font_buf.add(slice, m_region.x, m_region.y + display_density.device_to_logical(dev_line_height * line_off));
        m_font_buf.draw();
#else
    ...
    SizeReq Text::_get_preferred_size(Direction dim, int prosp_width)
    {
        ...
        if (!dim)
        {
            ...
            return {ellipsize ? min_ellipsized_width : wrap_text ? min_wrapped_width
                                                                 : w,
                    w};
        }
        else
    ...
    void Switcher::_allocate_region()
    {
        for (auto const &child : m_children)
        {
            ...
            case Widget::START:
                ...
                break;
            case Widget::CENTER:
                xo = (m_region.width - cr.width) / 2;
                break;
            case Widget::END:
                xo = m_region.width - cr.width;
                break;
            case Widget::STRETCH:
                xo = 0;
                break;
            ...
            case Widget::START:
                ...
                break;
            case Widget::CENTER:
                yo = (m_region.height - cr.height) / 2;
                break;
            case Widget::END:
                yo = m_region.height - cr.height;
                break;
            case Widget::STRETCH:
                yo = 0;
                break;
...
#define DIV_ROUND_UP(n, d) (((n) + (d) - 1) / (d))
    ...
    bool Scroller::on_event(const Event &event)
    {
        ...
        else if (event.type() == Event::Type::MouseDown && static_cast<const MouseEvent &>(event).button() == MouseEvent::Button::Left)
            delta = line_delta;
        if (delta != 0)
        {
    ...
    int Popup::base_margin()
    {
        ...
        return margin_small + (clipped - screen_small) * (margin_large - margin_small) / (screen_large - screen_small);
    }
#endif
    ...
    void UIRoot::debug_render()
    {
        ...
        if (debug_draw)
        {
            ...
            for (const auto &w : hover_path)
            {
                ...
                i++;
                VColour lc;
                lc = i == hover_path.size() ? VColour(255, 100, 0, 100) : VColour(0, 50 + i * 40, 0, 100);
        ...
        case Event::Type::KeyUp:
        {
            ...
            if (event.type() == Event::Type::KeyDown)
            {
                ...
                bool hotkey_handled = Widget::slots.hotkey.emit_if([&](Widget *w)
                                                                   { return top->is_ancestor_of(w->get_shared()); }, event);
                if (hotkey_handled)
                    return true;
    ...
    void UIRoot::recv_ui_state_change(const JsonNode *json)
    {
        ...
        if (!generation_id || generation_id->tag != JSON_NUMBER || generation_id->number_ != state.generation_id)
        {
            return;
        }
        ...
        if (!(widget_id->tag == JSON_STRING || widget_id->tag == JSON_NULL))
            return;
        const auto widget = widget_id->tag == JSON_STRING ? synced_widgets.at(widget_id->string_) : nullptr;
    ...
    void pump_events(int wait_event_timeout)
    {
        ...
        while (true)
        {
            ...
            if (event.type == WME_MOUSEMOTION && wm && wm->next_event_is(WME_MOUSEMOTION))
                continue;
            if (event.type == WME_KEYDOWN && (event.key.keysym.sym == 0 || event.key.keysym.sym == CK_NO_KEY))
            {
                continue;
            }
    ...
    {
        wm_mouse_event mev;
        mev.event = ev.type() == Event::Type::MouseMove ? wm_mouse_event::MOVE : ev.type() == Event::Type::MouseDown ? wm_mouse_event::PRESS
