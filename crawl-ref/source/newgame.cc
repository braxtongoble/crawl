...
{
    return _is_random_species(choice.species) && _is_random_job(choice.job);
}
...
static void _resolve_species_job(newgame_def &ng, const newgame_def &ng_choice)
{
    ...
    // to avoid bias.
    if (ng_choice.species == SP_VIABLE && ng_choice.job == JOB_VIABLE || ng_choice.species == SP_RANDOM && ng_choice.job == JOB_RANDOM)
    {
        spfirst = coinflip();
    }
                         ...
                         newgame_def defaults)
{
    ...
    {
        if (!yesno("Trunk doesn't count for the tournament, you want " TOURNEY ". Play trunk anyway? (Y/N)", false, 'n'))
        {
            game_ended(game_exit::abort);
        }
    }
    ...
    while (true)
    {
        ...
        else
        {
            ...
            {
                choice.species =
                    choice.allowed_species[random2(choice.allowed_species.size())];
            }
            ...
            {
                choice.weapon =
                    choice.allowed_weapons[random2(choice.allowed_weapons.size())];
            }
        }
                 ...
                 const newgame_def &defaults)
{
    ...
    ASSERT(is_good_name(ng.name, false) && _job_allowed(ng.species, ng.job) && ng.type != NUM_GAME_TYPE);
                                    ...
                                    UINewGameMenu *ng_menu)
{
    ...
    {
        if (ng.job == JOB_UNKNOWN || any_of(begin(group.species_list),
                                        ...
                                        UINewGameMenu *ng_menu)
{
    ...
    {
        if (ng.species == SP_UNKNOWN || any_of(begin(group.jobs), end(group.jobs), [&ng](job_type job)
...
public:
    UINewGameMenu(int _choice_type, newgame_def &_ng, newgame_def &_ng_choice, const newgame_def &_defaults) : m_choice_type(_choice_type), m_ng(_ng), m_ng_choice(_ng_choice), m_defaults(_defaults)
    {
        ...
        m_main_items->menu_id = m_choice_type == C_JOB ? "background-main" : "species-main";
        ...
        m_sub_items->menu_id = m_choice_type == C_JOB ? "background-sub" : "species-sub";
...
protected:
                                  ...
                                  const newgame_def &defaults)
    {
        ...
        if ((choice_type == C_SPECIES && ng.job != JOB_UNKNOWN) || (choice_type == C_JOB && ng.species != SP_UNKNOWN))
        {
            id = M_VIABLE;
        }
        else
        ...
        if ((choice_type == C_JOB && ng.species != SP_UNKNOWN) || (choice_type == C_SPECIES && ng.job != JOB_UNKNOWN))
        {
...
void UINewGameMenu::menu_item_activated(int id)
{
    ...
    default:
        ...
        if (m_choice_type == C_JOB)
        {
            ...
            if (m_ng.species == SP_UNKNOWN || _job_allowed(m_ng.species, job) != CC_BANNED)
            {
                m_ng_choice.job = job;
                done = true;
            }
        ...
        else
        {
            ...
            if (m_ng.job == JOB_UNKNOWN || species_allowed(m_ng.job, species) != CC_BANNED)
            {
                       ...
                       UINewGameMenu *ng_menu, menu_letter &letter)
{
    ...
    for (job_type &job : jobs)
    {
        ...
        if (ng.species != SP_UNKNOWN && _job_allowed(ng.species, job) == CC_BANNED)
        {
            continue;
        }
                           ...
                           UINewGameMenu *ng_menu, menu_letter &letter)
{
    ...
    for (species_type &this_species : species_list)
    {
        ...
        if (ng.job != JOB_UNKNOWN && species_allowed(ng.job, this_species) == CC_BANNED)
        {
            continue;
        }
                                   ...
                                   shared_ptr<OuterMenu> &sub_items)
{
    ...
    for (unsigned int i = 0; i < weapons.size(); ++i)
    {
        ...
        const auto bg = wpn_restriction == CC_UNRESTRICTED ? STARTUP_HIGHLIGHT_GOOD : STARTUP_HIGHLIGHT_BAD;
    ...
    if (defweapon != WPN_UNKNOWN)
    {
        ...
        text += defweapon == WPN_RANDOM ? "Random" : defweapon == WPN_VIABLE ? "Recommended"
                            ...
                            const vector<weap_choice> &weapons)
{
    ...
    {
        weapon =
            ng_choice.allowed_weapons[random2(ng_choice.allowed_weapons.size())];
    }
    ...
    case WPN_VIABLE:
    {
        ...
        for (weap_choice choice : weapons)
        {
            if (choice.second == CC_UNRESTRICTED && one_chance_in(++good_choices))
            {
                                         ...
                                         shared_ptr<OuterMenu> &sub_items)
{
    ...
    if (crawl_state.game_is_sprint() && !defaults.map.empty() && defaults.type == GAME_TYPE_SPRINT && _char_defined(defaults))
    {
        text.clear();
        text += "Tab - ";
        text += defaults.map;
...
{
    return m1->order < m2->order || m1->order == m2->order && m1->desc_or_name() < m2->desc_or_name();
}
                                 ...
                                 const newgame_def &defaults)
{
    ...
    {
        if (is_sprint && ng_choice.type == !crawl_state.sprint_map.empty())
        {
